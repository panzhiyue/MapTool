/**
 * @module EMap/Edit/VectorHelper
 */
// import * as FeatureHelper from 'EMap/Basic/FeatureHelper.js'
...
import { Style } from 'windicss/types/utils/style'
// import GeometryService from 'EMap/Task/GeometryService.js'
 ...
 * 矢量图层编辑类 
 */
class VectorHelper extends EditHelper {
    //要编辑的矢量图层 
    private layer_: VectorLayer<VectorSource>;
    //编辑主键 
    private maxEMapEditFeatureId_: number;

    private editStyle_: any;
    /**
     * 构造函数
     * @param {Object} opt_options
     * @param {module:ol/Map} [opt_options.map] 地图
     * @param {module:ol/layer/Vector} [opt_options.layer] 图层
     */
    ...
    KSBJ(callback, errorback) {
        if (this.editorBLL_.getIsLock()) {
            callback({ code: EditCode.LOCK, message: "正在执行其他操作！" });
            return;
        }
        ...
        callback({ code: EditCode.SUCCESS, message: "开始编辑！" });
    }
        ...
        if (this.editorBLL_.getEditLayer().getSource().getFeatures().length > 0 || this.editorBLL_.getRemoveFeatures().getLength() > 0) {
            if (confirm("是否保存!")) {
                this.save((result) => {
                    ...
                    this.refreshEditItem();
                    this.editorBLL_.setIsLock(false);
                    callback({ code: EditCode.SUCCESS, message: "保存编辑图斑！" });
                });
            } else {
                this.editorControl_.setIsEdit(false);
                this.clear();
                ...
                this.editorBLL_.setIsLock(false);
                callback({ code: EditCode.SUCCESS, message: "有编辑图斑,但是不保存！" });
            }
        } else {
            this.editorControl_.setIsEdit(false);
            this.clear();
            ...
            this.editorBLL_.setIsLock(false);
            callback({ code: EditCode.SUCCESS, message: "没有编辑图斑,直接结束！" });
        }
    }
    /**
     * @inheritdoc
    ...
    XQTB(feature, callback, errorback) {
        ...
        this.getFeaturesIntersectGeometry(this.editorBLL_.getEditLayer(), feature.getGeometry(), (clickFeature) => {
            //判断当前点击位置是否有编辑图斑,若有,则返回
            if (clickFeature && clickFeature.length > 0) {
                this.message("无法重复选择图斑!");

                this.editorBLL_.setIsLock(false);
                callback({ code: EditCode.WRANING, message: "无法重复选择图斑！" });
                return;
            }
            this.getFeaturesIntersectGeometry(this.layer_, feature.getGeometry(), (features) => {
                ...
                this.editorBLL_.setIsLock(false);
                callback({ code: EditCode.SUCCESS, message: "选取图斑成功！", features: features });
            });

            // let features = this.layer_.getSource().getFeaturesAtCoordinate(feature.getGeometry().getCoordinates());
    ...
    CJTB(feature, callback, errorback) {
        ...
        this.editorBLL_.setIsLock(false);
        callback({ code: EditCode.SUCCESS, message: "创建图斑成功！", feature: feature });
    }
    ...
    QGTB(feature, callback, errorback) {
        ...
        for (let i = 0; i < features.length; i++) {
            ...
            promiseArr.push(new Promise((resolve, reject) => {
                // this.geometryService_.cut(sourceFeature, lineFeature, (result) => {
                //     //相交
                //     if (result && result.status == "success" && result.result.geometry) {
                //         let wkt = new format.WKT();
                //         let geometry = wkt.readGeometry(result.result.geometry);
                //         if (geometry) {
        ...
        callback({ code: EditCode.SUCCESS, message: "切割图斑成功！" });
    }
    ...
    BJTB(features, callback, errorback) {
        ...
        this.editorBLL_.setIsLock(false);
        callback({ code: EditCode.SUCCESS, message: "编辑图斑成功！", features: features });
    }
    ...
    LRSX(feature, callback, errorback) {
        ...
        this.getFeaturesIntersectGeometry(this.editorBLL_.getEditLayer(), feature.getGeometry(), (features) => {
            if (features && features.length > 0) {
                console.log(features[0]);
            }
            this.editorBLL_.setIsLock(false);
            callback({ code: EditCode.SUCCESS, message: "录入属性！", features: features });
        });
    ...
    HBTB(callback, errorback) {
        ...
        this.editorBLL_.setIsLock(false);
        callback({ code: EditCode.SUCCESS, message: "合并图斑！", features: features });
    }
    ...
    SCTB(feature, callback, errorback) {
        ...
        this.getFeaturesIntersectGeometry(this.editorBLL_.getEditLayer(), feature.getGeometry(), (features) => {
            ...
            this.editorBLL_.setIsLock(false);
            callback({ code: EditCode.SUCCESS, message: "删除图斑！", features: features })
        });
    ...
    QXCZ(callback, errorback) {
        ...
        if (confirm("是否清除选择!")) {
            ...
            callback({ code: EditCode.SUCCESS, message: "清除选择！" });
        }
        this.editorBLL_.setIsLock(true);
    }
    ...
    BCTB(callback, errorback) {
        ...
        this.save((result) => {
            ...
            callback({ code: EditCode.SUCCESS, message: "保存！" });
        });
    }
    //#endregion  编辑对象接口
    ...
    refreshEditItem() {
        ...
        if (this.editorControl_.getIsEdit() == true) {
            if (addFeatures.length == 0) {
                if (removeFeatures.length == 0) {
                    enableTypes = [EditType.STOP, EditType.SELECT, EditType.CREATE];
                    hideTypes = [EditType.START];
                } else {
                    enableTypes = [EditType.STOP, EditType.SELECT, EditType.SAVE];
                    hideTypes = [EditType.START];
                }
            }
            else if (addFeatures.length == 1) {
                enableTypes = [EditType.STOP, EditType.SELECT, EditType.CREATE, EditType.CUTTING, EditType.EDIT, EditType.DELETE, EditType.CANCEL, EditType.SAVE];
                hideTypes = [EditType.START];
            } else {
                enableTypes = [EditType.STOP, EditType.SELECT, EditType.CREATE, EditType.CUTTING, EditType.EDIT, EditType.DELETE, EditType.CANCEL, EditType.SAVE, EditType.MERGE];
                hideTypes = [EditType.START];
            }
        ...
        //不在编辑
            ...
            enableTypes = [EditType.START];
            hideTypes = [EditType.STOP];
        }
        ...
        for (let i = 0; i < reverse.length; i++) {
            ...
            if (itemInfo && (itemInfo.state == EditState.ENABLE || itemInfo.state == EditState.SELECTED)) {
                this.editorControl_.updateItemState(reverse[i], EditState.DISABLE);
            }
        }
    ...
    getStyle(feature, index) {
        if (this.editorBLL_.getType() == EditType.MERGE) {
