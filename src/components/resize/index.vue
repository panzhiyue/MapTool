<template>
	<div class="resize" :class="classes" :style="styles">
		<slot></slot>
		<div v-if="helper && resizing" class="resize__helper" :style="helperStyle"></div>
		<drag
			v-if="!disabled && (!axis || axis === 'h')"
			ref="ctrlH"
			class="resize__ctrl-h"
			axis="h"
			:origin="getOrigin"
			@start="handleStart"
			@drag="handleDragH"
