				...
				<template #bodyCell="{ value, index, column }">
					...
					<template v-if="column.key == 'type'">
						<sqlite-column-type
							class="w-11/12"
							:value="value"
							@change="(e) => handleChange(e, index, column)"></sqlite-column-type>
					</template>
					...
					<template v-if="column.key == 'primary'">
						<div class="w-full h-30px" @click="handleChange(!value, index, column)">
							<i v-if="value" class="icon icon-primary text-30px text-yellow-500"></i>
						</div>
					</template>
				</template>
			</a-table>
