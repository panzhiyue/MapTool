<template>
	<div class="drag" :class="classes">
		<slot></slot>
	</div>
</template>
<script>
import { on, off, once, addClass, removeClass, setStyle, getStyle } from '@/utils/dom';
...
// 拖拽句柄样式名
const HANDLE_CLASS = 'drag__handle';
...
export default {
	...
	methods: {
		...
		createDragEl(e) {
			...
			addClass(this.dragEl, 'drag__clone');
			if (this.cloneClass) {
		...
		// 是否有resize子组件正在resizing
		isResizing() {
			return false;
			// return !!this.$el.children.find((item) => {
			// 	if (item.$options && item.$options.name === 'Resize') {
		...
		userSelect(disabled) {
			disabled
				? addClass(this.document.body, 'select-none')
				: removeClass(this.document.body, 'select-none');
		},
		// 拖拽开始
		start(e) {
			this.cacheRange = null;
			// 标识正在拖拽
			this.dragging = true;
			// 初始化已放置，开始是未放置，这个属性的修改是在 drop 组件中修改为true
			this.dropped = false;
			this.createDragEl(e);
			this.updateOffset(e);
		...
		stop() {
			 ...
			 */
			this.$emit('stop', this);
			//   bus.$emit("drag:stop", this);
			// 已过成功放置，清除拖拽副本，否则就重置，dropped 在drop中更新，这行要放在触发stop事件之后
...
.drag__clone.is-clone {
	position: absolute !important;
}
.drag__clone.is-revert {
