...
export default {
	...
	methods: {
		init() {
			...
			ipcRenderer.removeAllListeners('ipcSendMsg');
			ipcRenderer.on('ipcSendMsg', (event, result) => {
				console.log('[ipcRenderer] [socketMsgStart] result:', result);

				this.messageString = result;
				// 调用后端的另一个接口
				event.sender.send('hello', 'electron-egg');
			});

			// 监听 窗口2 发来的消息
			ipcRenderer.removeAllListeners('window2ToWindow1');
			ipcRenderer.on('window2ToWindow1', (event, arg) => {
				alert(arg);
			});
		},
		sendMsgStart() {
			...
			ipcRenderer.send('ipcSendMsg', params);
		},
		sendMsgStop() {
			const params = {
				type: 'end',
		...
		handleInvoke() {
			ipcRenderer.invoke('ipcInvokeMsg', '异步-回调').then((r) => {
				console.log('r:', r);
				this.message1 = r;
			});
		},
		async handleInvoke2() {
			const msg = await ipcRenderer.invoke('ipcInvokeMsg', '异步');
			console.log('msg:', msg);
			this.message2 = msg;
		},
		handleSendSync() {
			const msg = ipcRenderer.sendSync('ipcSendSyncMsg', '同步');
			this.message3 = msg;
		},
		createWindow(index) {
			ipcRenderer.invoke('createWindow', this.views[index]).then((id) => {
				console.log('[createWindow] id:', id);
			});
		},
		async sendTosubWindow() {
			// 新窗口id
			this.newWcId = await ipcRenderer.invoke('getWCid', this.windowName);
			ipcRenderer.sendTo(this.newWcId, 'window1ToWindow2', '窗口1通过 sendTo 给窗口2发送消息');
		},
	},
};
</script>
